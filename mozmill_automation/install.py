...
class Installer(object):
    ...
    def install(self, package=None, destination="./"):
        ...
        # Linux
            ...
            cmdArgs = ["hdiutil", "attach", "-noautoopen",
                       "-mountpoint", mountpoint, package]
            result = subprocess.call(cmdArgs)
            ...
            if not result:
                ...
                try:
                    timeout = 20
                    files = glob.glob(mountpoint + '/*.app')

                    # hdiutil can return earlier so we have to wait a bit for
                    # files to appear at the mounted location
                    while not files:
                        time.sleep(1)
                        timeout -= 1

                        if timeout == 0:
                            raise TimeoutError("No application found in '%s'." % (fileName))
                    ...
                    cmdArgs = ["hdiutil", "detach", mountpoint, "-force"]
                    result = subprocess.call(cmdArgs)
